...
import { BillingDetailReportState, loadMasterData, loadUserDetail, updateField } from './BillingDetailReport.slice';
...
import { downloadBillingDetailReport } from '../../../../../../services/invoiceReport';

const BillingDetailReport = () => {
    const { t } = useTranslation();
    const {
        billingdetailreport: { BillingReportFilter, MasterData, loggeduserinfo },
    } = useStore(({ billingdetailreport }) => ({ billingdetailreport }));

    useEffect(() => {
        const fetchData = async () => {
                const TenantLocations = await getRegionAndCategoryWiseWiseTenantOfficeList(String(BillingReportFilter.TenantRegionId));
        ...
        fetchData();
    }, [BillingReportFilter.TenantRegionId]);
    ...
    useEffect(() => {
        const fetchCustomers = async () => {
            const Customers = await getCustomerNamesByLocationGroupFilter(BillingReportFilter.TenantOfficeId);
        ...
        fetchCustomers();
    }, [ BillingReportFilter.TenantOfficeId,BillingReportFilter.CustomerGroupId]);
    ...
    const onDownloadClick = async () => {
        const response = await downloadBillingDetailReport(BillingReportFilter);
    ...
    function onFieldChangeSelect(selectedOption: any, actionMeta: any) {
        ...
        store.dispatch(updateField({ name: name as keyof BillingDetailReportState['BillingReportFilter'], value }));
    }
                ...
                <div className="">
                    <div className='row mb-2'>
                        <div className='col-md-6'>
                            <div className="">
                                <label className='text-size-13'>{t('billingdetailreport_date_from_label')}</label>
                                <input
                                    name='DateFrom'
                                    value={BillingReportFilter.DateFrom ? BillingReportFilter.DateFrom : ""}
                        ...
                        <div className='col-md-6'>
                            <div className="">
                                <label className='text-size-13'>{t('billingdetailreport_date_to_label')}</label>
                                <input
                                    name='DateTo'
                                    value={BillingReportFilter.DateTo ? BillingReportFilter.DateTo : ""}
                                    onChange={onUpdateField}
                                    type='date'
                                    className='form-control'
                                ></input>
                    ...
                    <div className="row">
                        <div className='mt-2 col-md-6'>
                            <label className='text-size-13'>{t('billingdetailreport_tenant_region_label')}</label>
                            <div className="">
                                <Select
                                    options={MasterData.TenantRegions}
                                    value={MasterData.TenantRegions && MasterData.TenantRegions.find(option => option.value == BillingReportFilter.TenantRegionId) ||''}
                                    isSearchable
                        ...
                        <div className='mt-2 col-md-6'>
                            <label className='text-size-13'>{t('billingdetailreport_tenant_location_label')}</label>
                            <div className="">
                                <Select
                                    options={MasterData.TenantOffices}
                                    value={MasterData.TenantOffices && MasterData.TenantOffices.find(option => option.value == BillingReportFilter.TenantOfficeId) || ''}
                                    isSearchable
                                    onChange={onFieldChangeSelect}
                                    name='TenantOfficeId'
                                />
                    ...
                    <div className="row">
                        <div className='mt-2 col-md-6'>
                            <label className='text-size-13'>{t('billingdetailreport_customergroup_label')}</label>
                            <div className="">
                                <Select
                                    options={MasterData.CustomerGroups}
                                    value={MasterData.CustomerGroups && MasterData.CustomerGroups.find(option => option.value == BillingReportFilter.CustomerGroupId) || ''}
                                    isSearchable
                                    onChange={onFieldChangeSelect}
                        ...
                        <div className='mt-2 col-md-6'>
                            <label className='text-size-13'>{t('billingdetailreport_customer_label')}</label>
                            <div className="">
                                <Select
                                    options={MasterData.Customers}
                                    value={MasterData.Customers && MasterData.Customers.find(option => option.value == BillingReportFilter.CustomerId) || ''}
                                    isSearchable
                                    onChange={onFieldChangeSelect}
                                    name='CustomerId'
                                />
                    ...
                    >
                        {t('billingdetailreport_download_button')}
                    </button>
                </div>
            </div>
        </div >
