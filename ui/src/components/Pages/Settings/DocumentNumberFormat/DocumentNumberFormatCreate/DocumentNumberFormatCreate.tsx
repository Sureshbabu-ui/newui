...
import { initializeDocumentNumberFormatCreate, updateField, updateErrors, toggleInformationModalStatus, DocumentNumberFormatCreateState, loadDocTypes, setProceed, setCustomString, NumFormatupdateField, createFormatArray, SampleNumFormatupdateField, createFormat, clearFormatArray } from './DocumentNumberFormatCreate.slice'
...
import { createDocumentNumberFormat, getDocumentNumberFormatList } from '../../../../../services/documentnumberformat';
...
export const DocumentNumberFormatCreate = () => {
  const modalRef = useRef<HTMLButtonElement>(null);
  const { t } = useTranslation();
  const [IsCustomString, setIsCustomString] = useState(false)
  const [submit, setEnableSubmit] = useState(false);
  const [customStringError, setcustomStringError] = useState("");
  const { displayInformationModal, errors, numberformat, DocumentTypes, NumFormatArray, SampleDocumentFormat, IsProceed, CustomString } = useStore(({ numberformatcreate }) => numberformatcreate)
  ...
  const onLoad = async () => {
    try {
      store.dispatch(initializeDocumentNumberFormatCreate())
  ...
  const validationSchema = yup.object().shape({
    separator: yup.string().nullable().matches(/^[!@#$%^&*()_+/\-=;':"\\|,.<>?`~[\]]+$/, {
      message: t('validation_error_documentnumberformat_create_separator_valid_required') ?? '',
      excludeEmptyString: true,
    }),
    DocumentTypeId: yup.number().positive('validation_error_documentnumberformat_create_dctype_required'),
    NumberPadding: yup.number().moreThan(0, 'validation_error_documentnumberformat_create_numpadding_required').typeError('validation_error_documentnumberformat_create_numpadding_required'),
    documentNumber: yup.number().moreThan(0, 'validation_error_documentnumberformat_create_documentnumber_required').typeError('validation_error_documentnumberformat_create_documentnumber_required').max(5, ('validation_error_documentnumberformat_create_documentnumber_amcvalue_exceeds'))
  });

  const validationFormatSchema = yup.object().shape({
    Format: yup.string().max(16, t('validation_error_documentnumberformat_create_format_valid_required') ?? '')
  });

  useEffect(() => {
    store.dispatch(createFormatArray(true))
  }, [numberformat.documentNumber])


  const onModalClose = () => {
    store.dispatch(initializeDocumentNumberFormatCreate())
    onLoad()
    setIsCustomString(false)
  ...
  const onSubmit = async () => {
    ...
    if (checkCustomString() == false) {
      ...
      const result = await createDocumentNumberFormat(store.getState().numberformatcreate.numberformat);
      result.match({
        ok: () => {
  ...
  const onUpdateField = (ev: any) => {
    ...
    store.dispatch(updateField({ name: name as keyof DocumentNumberFormatCreateState['numberformat'], value }));
  }
  ...
  const InformationModal = () => {
    ...
    return (
      <SweetAlert success title='Success' onConfirm={reDirectRoute}>
        {t('documentnumberformat_create_success')}
      </SweetAlert>
    );
  }
  const reDirectRoute = async () => {
    ...
    document.getElementById('closeCreateNumberFormatModal')?.click();
  }
        ...
        <div className="modal-dialog  modal-xl">
          <div className="modal-content">
            <div className="modal-header mx-2">
              <h5 className="modal-title app-primary-color">{t('documentnumberformat_create_modal')}</h5>
              <button
                type='button'
                className="btn-close"
                data-bs-dismiss='modal'
                id='closeCreateNumberFormatModal'
                aria-label='Close'
            ...
            <div className="modal-body">
              ...
              <div className="row">
                <div className="d-flex bd-highlight">
                  <div className="p-2 flex-grow-1 bd-highlight red-asterisk">{t('documentnumberformat_create_dctype')}</div>
                ...
                <div className="d-flex bd-highlight">
                  <div className="p-2 flex-grow-1 bd-highlight red-asterisk">{t('documentnumberformat_create_document_number')}</div>
                  <div className="p-2 bd-highlight col-md-3">
                ...
                <div className="d-flex bd-highlight">
                  <div className="p-2 flex-grow-1 bd-highlight red-asterisk">{t('documentnumberformat_create_running_number')}</div>
                  <div className="p-2 bd-highlight col-md-3">
                ...
                <div className="d-flex bd-highlight">
                  <div className="p-2 flex-grow-1 bd-highlight">
                    <span>{t('documentnumberformat_create_delimiter')}</span>
              ...
              {IsProceed == false &&
                <div className="d-flex flex-row-reverse bd-highlight">
                  <div className="p-2 bd-highlight">
                    <button type="button" onClick={onProceed} className="btn  app-primary-bg-color text-white mt-2 float-end">{t('documentnumberformat_create_proceed_button')}</button>
                  </div>
                </div>
              }
              {IsProceed == true &&
                <>
                  <div className="row mx-2 mt-3">
                    <div className="d-flex bd-highlight p-0 m-0">
                      <div className="pt-1 ps-0 flex-grow-1 bd-highlight app-primary-color fw-bold">{t('documentnumberformat_create_sub_heading')}</div>
                    </div>
                    <div className="row mt-2 mx-0 p-0">
                      {Array.from({ length: 2 * numberformat.documentNumber - 1 }).map((_, rowIndex) => (
                        <>
                          <div key={rowIndex} className={`col ps-0 pe-2 mb-3 ${rowIndex % 2 == 0 ? "col-md-5" : " col-md-3"}`}>
                            {rowIndex % 2 === 0 ? (
                              <Select
                                ...
                                placeholder={`${t('documentnumberformat_create_placeholder_select_format')}`}
                                className="w-100"
                              />
                            ) : (
                              <>
                                {numberformat.separator != "" &&
                                  <Select
                                    ...
                                    placeholder={`${t('documentnumberformat_create_placeholder_select_separator')}`}
                                    className="w-100"
                                  />
                                }
                              </>
                          ...
                          {(IsCustomString && NumFormatArray.some(item => (item.value === "{CUSTSTR}" || (CustomString !== "" && item.value === CustomString)) && item.key === rowIndex + 1)) ? (
                            <div className="col-md-3 mb-2 ps-0">
                              <input onChange={(ev) => onFormatUpdateField(ev, rowIndex + 1)} maxLength={3} name="CustomString" placeholder={`${t('documentnumberformat_create_custom_string')}`} value={CustomString} className={`form-control  ${customStringError != "" ? "border border-danger" : ""}`}></input>
                  ...
                  <div className="mx-2 p-1 mt-4 bg-light">
                    <div>{t('documentnumberformat_create_sample')}</div>
                  ...
                  <div className="d-flex flex-row-reverse bd-highlight">
                    <div className="bd-highlight pe-2">
                      <button type="button" onClick={onSubmit} disabled={submit == false} className="btn app-primary-bg-color text-white mt-2 float-end">{t('documentnumberformat_create_submit_button')} </button>
                      <button type="button" onClick={onResetFormat} className="btn  app-primary-bg-color text-white float-end mt-2 me-2">{t('documentnumberformat_create_reset_button')}</button>
                    </div>
                  </div>
                </>
              }
