...
import { changePage, initializePartIndentDemand, loadPartIndentDemand, setSearch } from './PartIndentDemandCompleted.slice';
...
const PartIndentDemandsCompleted = () => {
    const { t } = useTranslation();
    const breadcrumbItems = () => {
        return [{ Text: 'breadcrumbs_home', Link: '/' },
        { Text: 'breadcrumbs_manage_demandnote_requests' }];
    }

    const { demandscwhnotneeded, currentPage, search, totalRows, perPage } = useStore(
        ({ partindentdemandlogisticsnotallocated }) => partindentdemandlogisticsnotallocated
    );
    ...
    const onLoad = async () => {
        ...
        try {
            const result = await DemandListCWHAttentionNotNeeded(currentPage, store.getState().partindentdemandlogisticsnotallocated.search, false);
    ...
    async function onPageChange(index: number) {
        ...
        const result = await DemandListCWHAttentionNotNeeded(index, search, false);
        store.dispatch(loadPartIndentDemand(result));
    }
    ...
    const addData = async (event: any) => {
        ...
        if (event.target.value == "") {
            ...
            const result = await DemandListCWHAttentionNotNeeded(store.getState().partindentrequestlist.currentPage, store.getState().partindentrequestlist.search, false);
            store.dispatch(loadPartIndentDemand(result));
        }
    }
    ...
    async function filterPartIndentRequests(event: any) {
        store.dispatch(changePage(1))
        const requests = await DemandListCWHAttentionNotNeeded(1, search, false);
        store.dispatch(loadPartIndentDemand(requests));
    }

    async function setDemandInfo(PartIndentDemandId: number, TenantOfficeId: number, WorkOrderNumber: string, Quantity: number, StockTypeId: number) {
        store.dispatch(loadDemandData({ PartIndentDemandId: PartIndentDemandId, TenantOfficeId: TenantOfficeId, WorkOrderNumber: WorkOrderNumber, Quantity: Quantity, StockTypeId: StockTypeId }))
    }
                                                    ...
                                                    <tbody>
                                                        {demandscwhnotneeded.map((field, index) => (
                                                            <tr className="mt-2" key={index}>
                                                                <td>{field.indentdemand.AllocatedOn == null && field.indentdemand.CLPartCount && field.indentdemand.CLPartCount >= Math.floor(field.indentdemand.Quantity) ? (
                                                                    <a
                                                                        type="button"
                                                                        onClick={() => { setDemandInfo(field.indentdemand.Id, field.indentdemand.TenantOfficeId, field.indentdemand.WorkOrderNumber, field.indentdemand.Quantity, field.indentdemand.StockTypeId) }}
                                                                        className="pseudo-href app-primary-color" data-bs-toggle="modal"
                                                                        data-bs-target="#CreateGIN">
                                                                        {t('partindentdemand_allocate_button')}
                                                                    </a>
                                                                ) : field.indentdemand.AllocatedOn !== null ? (
                                                                    ...
                                                                    <a className="pseudo-href app-primary-color" href={`/logistics/partindentdemands/detail/${field.indentdemand.Id}`} data-toggle="tooltip" data-placement="left" title={'View'} >
                                                                        <span className="material-symbols-outlined">
                                                                            visibility
                                                                        </span>
                                                                    </a>
                                                                ...
                                                                <td>
                                                                    {field.indentdemand.AllocatedOn == null && field.indentdemand.CLPartCount && field.indentdemand.CLPartCount >= Math.floor(field.indentdemand.Quantity) ? (
                                                                        <p>Waiting Allocation </p>
                                                                    ) : field.indentdemand.AllocatedOn !== null ? (
                                                                        <p>Allocated</p>
                                                                    ) : (
                                                                        <p>Under Process</p>
...
export default PartIndentDemandsCompleted