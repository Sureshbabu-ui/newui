...
import { EditState, initializeStateEdit, toggleInformationModalStatus, updateErrors, updateField } from "./StateEdit.slice";
...
import { getAllStates, stateEdit } from "../../../../../services/state";
...
export const StateEdit = () => {
    ...
    const onUpdateField = (ev: any) => {
        ...
        store.dispatch(updateField({ name: name as keyof EditState['state'], value }));
    }
    ...
    const onSubmit = async () => {
        ...
        const result = await stateEdit(state)
        result.match({
            ok: () => {
    ...
    const InformationModal = () => {
        ...
        return (
            <SweetAlert success title="Success" onConfirm={updateState}>
                {t('state_update_success_message')}
            </SweetAlert>
        );
    }
    ...
    const validationSchema = yup.object().shape({
        Name: yup.string().required('validation_error_state_update_name_required'),
        CountryId: yup.number().positive('validation_error_state_update_country_required'),
        GstStateCode: yup.string().required('validation_error_state_update_gst_state_code_required'),
        GstStateName: yup.string().required('validation_error_state_update_gst_state_name_required'),
    });
                ...
                <div className="modal-dialog ">
                    <div className="modal-content">
                        <div className="modal-header mx-2">
                            <h5 className="modal-title">{t('state_update_title')}</h5>
                            <button
                                type='button'
                                className="btn-close"
                                data-bs-dismiss='modal'
                                id='closeEditStateModal'
                                aria-label='Close'
