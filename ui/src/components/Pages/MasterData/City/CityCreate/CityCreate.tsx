...
import { CreateCityState, initializeCityCreate, loadStates, loadTenantOffices, setCountryId, toggleInformationModalStatus, updateErrors, updateField } from "./CityCreate.slice";
...
import { cityCreate, getAllCities } from "../../../../../services/city";
...
import { MasterData } from './../../../ServiceRequestManagement/InterimRequestManagement/InterimRequestReview/InterimRequestReview.slice';

export const CityCreate = () => {
    const { t } = useTranslation();
    const modalRef = useRef<HTMLButtonElement>(null);
    const { citycreate: { CountryId, city, States, displayInformationModal, errors, Location } } = useStore(({ citycreate }) => ({ citycreate }));

    const [countryList, setCountryList] = useState<any>(null)
    const [formattedCountryList, setFormattedCountryList] = useState<any>(null)

    useEffect(() => {
        store.dispatch(initializeCityCreate())
        getCountryList()
        onLoad()
    }, [])
    ...
    const onSubmit = async () => {
        ...
        const result = await cityCreate(city)
        result.match({
            ok: () => {
    ...
    const validationSchema = yup.object().shape({
        Code: yup.string().required('validation_error_city_create_code_required').max(8, 'validation_error_city_create_code_max'),
        Name: yup.string().required('validation_error_city_create_name_required'),
        StateId: yup.number().positive('validation_error_city_create_state_required'),
                ...
                <div className="modal-dialog ">
                    <div className="modal-content">
                        <div className="modal-header mx-2">
                            <h5 className="modal-title">{t('city_create_title')}</h5>
                            <button
                                type='button'
                                className="btn-close"
                                data-bs-dismiss='modal'
                                id='closeCreateCityModal'
                                aria-label='Close'
                                    ...
                                    <div className=''>
                                        <div className='row mb-1'>
                                            <div className='col-md-12'>
                                                <label className="mt-2 red-asterisk">{t('city_create_input_code')}</label>
                                                <input name='Code'
                                                    value={city.Code}
                                            ...
                                            <div className='col-md-12'>
                                                <label className="mt-2 red-asterisk">{t('city_create_input_name')}</label>
                                            ...
                                            <div className="col-md-12">
                                                <label className="mt-2 red-asterisk">{t('city_create_input_country')}</label>
                                                <Select
                                                    ...
                                                    name="CountryId"
                                                    placeholder={t('city_create_placeholder_country')}
                                                />
                                                <div className="small text-danger"> {t(errors['CountryId'])}</div>
                                            </div>
                                            <div className=" mt-2">
                                                <label className="form-label  mb-0 red-asterisk">{t('city_create_label_state_id')}</label>
                                                <Select
                                                    options={States}
                                                    name="StateId"
