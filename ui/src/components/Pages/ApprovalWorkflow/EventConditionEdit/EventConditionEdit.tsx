...
import { addCondition, clearWorkflowDetail, EditEventConditionState, loadApprovalWorkflows, loadEventConditionDetails, loadEventConditionMasters, loadMasterData, removeCondition, setSelectedColumn, toggleInformationModalStatus, updateCondition, updateErrors, updateField } from "./EventConditionEdit.slice";
...
import { editEventCondition, getEventConditionListView } from "../../../../services/ApprovalWorkflow/eventCondition";
...
const EventConditionEdit = () => {
    const { EventId } = useParams<{ EventId: string }>();
    const { EventConditionId } = useParams<{ EventConditionId: string }>();

    const { t } = useTranslation();

    const {
        eventconditionedit: { eventConditionMasterData, approvalWorkflows, eventCondition, masterDataList, selectedConditionArray, columns, selectedColumn, errors, displayInformationModal, eventDetail },
    } = useStore(({ eventconditionedit, app }) => ({ eventconditionedit, app }));

    useEffect(() => {
        if (checkForPermission('APPROVALWORKFLOW_MANAGE') && EventConditionId)
            store.dispatch(updateField({ name: 'EventConditionId', value: EventConditionId }));
    }, [EventConditionId]);

    useEffect(() => {
        if (checkForPermission('APPROVALWORKFLOW_MANAGE') && eventCondition.EventConditionId)
            onLoad()
    }, [eventCondition.EventConditionId])
    ...
    const onSelectChange = (selectedOption: any, actionMeta: any) => {
        ...
        store.dispatch(updateField({ name: name as keyof EditEventConditionState['eventCondition'], value }));
    }
    ...
    const validationSchema = yup.object().shape({
        ConditionName: yup.string().required('validation_error_eventconditionedit_workflow_required'),
        ApprovalWorkflowId: yup.string().required('validation_error_eventconditionedit_workflow_required'),
    });
    ...
    const InformationModal = () => {
        ...
        return (
            <SweetAlert success title="Success" onConfirm={handleReload}>
                {t('eventconditionedit_success_message')}
            </SweetAlert>
        );
    }
    ...
    const onSubmit = async () => {
        ...
        store.dispatch(startPreloader());
        const result = await editEventCondition(EventConditionId, eventCondition);
        result.match({
            ok: () => {
    ...
    const breadcrumbItems = () => {
        return [
            ...
            { Text: 'breadcrumbs_manage_eventconditionedit' }
        ];
    }
        ...
        <div>
            ...
            <div className="row">
                <div className="col-md-6 mt-2 pt-3">
                    <div className="mb-2">
                        {`${t('eventconditionedit_title_eventgroup')}: ${eventDetail.EventGroupName}`}
                    </div>
                    <div className="mb-2">
                        {`${t('eventconditionedit_title_eventname')}: ${eventDetail.EventName}`}
                    </div>
                </div>
            </div>
                ...
                <div className="col-md-7">
                    <div className="row">
                        <div className="col-md-8">
                            <div className="mb-3">
                                <label className="red-asterisk">{t('eventconditionedit_input_conditionname')}</label>
                            ...
                            <div className="mb-3">
                                <label className="red-asterisk">{t('eventconditionedit_input_workflow')}</label>
                                <Select
                                    ...
                                    placeholder={t('eventconditionedit_select_workflow')}
                                    classNames={{
                                        control: (state) =>
                    ...
                    {columns.length > 0 ? <div className="row">
                        <div className="col-md-8">
                            <Select
                                options={columns}
                                ...
                                isClearable
                                name="selectedColumn"
                                placeholder={t('eventconditionedit_select_column')}
                            />
                        </div>
                                            ...
                                            <div className="row px-0 py-2 pt-1 bg-white">
                                                ...
                                                <div className="col-md-3">
                                                    <Select
                                                        ...
                                                        isClearable
                                                        name={event.key}
                                                        placeholder={t('eventconditionedit_select_masterdata')}
                                                    />
                                                </div>
            ...
            <div className="row">
                <div className="col-md-6">
                    <button type='button' className='btn  app-primary-bg-color text-white w-75 mb-3' onClick={onSubmit}>
                        {t('eventconditionedit_submit_button')}
                    </button>
                </div>
                <div className="col-md-6">
...
export default EventConditionEdit