...
import { Configurations, SelectDetails, Select, ContractRenewDetail } from '../../../../../types/contract';
import { SelectedContractData } from '../../../../../types/contract';
...
export interface ContractRenewState {
  masterDataList: Configurations;
  contract: ContractRenewDetail;
...
const initialState: ContractRenewState = {
  masterDataList: {
    AgreementType: [],
    BookingType: [],
    ServiceMode: [],
...
const slice = createSlice({
  ...
  reducers: {
    ...
    loadMasterData: (state, { payload: { name, value: { Select } } }: PayloadAction<{ name: keyof ContractRenewState['masterDataList']; value: SelectDetails }>) => {
      state.masterDataList[name] = Select.map((masterData) => (masterData))
      if (name == 'PaymentType') {
        state.masterDataList['PaymentType'] = state.masterDataList['PaymentType'].filter(item => !state.appvalues.AppValue.includes(item.value))
      }
    },
    ...
    updateField: (
      state,
      { payload: { name, value } }: PayloadAction<{ name: keyof ContractRenewState['contract']; value: any }>
    ) => {
      switch (name) {
        case 'IsMultiSite':
          if (value === false) {
