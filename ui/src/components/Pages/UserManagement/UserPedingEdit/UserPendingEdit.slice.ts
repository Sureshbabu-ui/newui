...
import { DesignationSelectDetails, MasterDataItems, MastersConfiguration, MultipleUserRoles, SelectDetails, SelectTenantOffice, UserEdit, UserRole } from '../../../../types/user';
...
export interface EditUserState {
  ...
  States: valuesInMasterDataByTableDetailsSelect[],
  Cities: valuesInMasterDataByTableDetailsSelect[],
  Countries: valuesInMasterDataByTableDetailsSelect[],
  selectedRoles: number[];
  Customer: valuesInMasterDataByTableDetailsSelect[],
  ContractNumbers: valuesInMasterDataByTableDetailsSelect[];
  CustomerSiteNames: valuesInMasterDataByTableDetailsSelect[];
}
...
const slice = createSlice({
  ...
  reducers: {
    ...
    loadMasterData: (state, { payload: { name, value: { Select } } }: PayloadAction<{ name: keyof EditUserState['masterDataList']; value: SelectDetails }>) => {
    ...
    loadContractNumbers: (state, { payload: { MasterData } }: PayloadAction<valuesInMasterDataByTableSelect>) => {
      state.ContractNumbers = MasterData.map((ContractNumbers) => ContractNumbers);
    },
    loadCustomerSites: (state, { payload: { MasterData } }: PayloadAction<valuesInMasterDataByTableSelect>) => {
      state.CustomerSiteNames = MasterData.map((CustomerSiteNames) => CustomerSiteNames);
    },
  },
});
