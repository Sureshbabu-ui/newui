...
import { editUser, getUserCategoryFilteredTenantOfficeName, getUserTenantOfficeName, getUserDetails, getUserRoles, getUsersRolesList, getUserBusinessUnits, editUserPendingApproval, getUserPendingDetails } from '../../../../services/users';
...
export function UserPendingEdit() {
  ...
  const { errors, roles, user, updatedRoles, ContractNumbers, Customer, CustomerSiteNames, Cities, Countries, States, entitiesList, displayInformationModal, TenantOffices, masterDataList } =
    useStoreWithInitializer(({ userpendingedit }) => userpendingedit, GetMasterDataItems);
  ...
  const [engCategory, setEngCategory] = useState('ETP_RENG')
  ...
  const appValues = (store.getState().userpendingedit?.appvalues?.AppKey === "SEDesignationCodes") ? (store.getState().userpendingedit.appvalues.AppValue.split(",")) : [];
  ...
  const validationSchema = yup.object().shape({
    ...
    DocumentFile: yup.mixed<FileList>()
      .test('fileFormat', 'validation_error_edit_user_create_file_type_mismatch', (value: any) => {
        if (!value) return true; // Skip validation if no file is provided
        return process.env.REACT_APP_USER_DOCUMENT_TYPES?.split(",").includes(value.type);
      })
      .test('fileSize', 'validation_error_edit_user_create_file_maxsize', (value: any) => {
        if (!value) return true; // Skip validation if no file is provided
  ...
  async function onLoad() {
    ...
    try {
      if (Number(PendingUserId) > 0) {
        const tableName = "AWF_USER_CREATE"
        var { UserPendingDetail } = await getUserPendingDetails(PendingUserId);
        var parsedUserDetails: UserEdit = JSON.parse(UserPendingDetail.Content)
  ...
  const getFilteredContracts = async () => {
    ...
    store.dispatch(loadContractNumbers({ MasterData: FormatedContracts }));
  }
  ...
  const fetchData = async () => {
    try {
      ...
      store.dispatch(loadCustomerSites({ MasterData: SiteNames }));
    } catch (error) {
      console.error(error);
    }
  };
  ...
  function isRoleAlreadyAssigned(roleId: number) {
    const userRoles = store.getState().userpendingedit.user.UserRoles;
              ...
              <form>
                ...
                <div className='mb-3'>
                  ...
                  <Select
                    ...
                    name='GenderId'
                    placeholder={t('create_user_label_gender')}
                  />
                  <div className='small text-danger'> {errors['GenderId']}</div>
                </div>
                <div className='mb-3'>
                  <label className='red-asterisk'>{t('validation_error_edit_user_gender_required')}</label>
                  <Select
                    ...
                    placeholder={t('validation_error_edit_user_engagement_type_required')}
                  />
                ...
                <div className='mb-3'>
                  ...
                  <Select
                    ...
                    isSearchable
                    name='DepartmentId'
                    placeholder={t('validation_error_edit_user_department_required')}
                  />
                ...
                <div className='mb-3'>
                  ...
                  <Select
                    ...
                    isSearchable
                    name='DivisionId'
                    placeholder={t('validation_error_edit_user_division_required')}
                  />
                ...
                <div className='mb-3'>
                  ...
                  <Select
                    ...
                    placeholder={t('validation_error_edit_user_reporting_manager_required')}
                  />
                ...
                <div className='mb-3'>
                  ...
                  <Select
                    ...
                    isSearchable
                    name='DesignationId'
                    placeholder={t('validation_error_edit_user_designation_required')}
                  />
                ...
                <div className='mb-3'>
                  ...
                  />
                  <div className='small text-danger'> {errors['UserGradeId']}</div>
                </div>
                ...
                {isServiceEngineer && (
                  <>
                        ...
                        <div>
                          <div className="pb-2">
                            ...
                            <Select
                              ...
                              onChange={onFieldChangeSelect}
                              isSearchable
                              placeholder={t("edit_user_placeholder_contractnumber")}
                          ...
                          <div className="mb-1">
                            ...
                            <input type="date" name="StartDate" onChange={onUpdateField} value={user.StartDate ? formatDateTime(user.StartDate.toString()) : ""} className="form-control"></input>
                          </div>
                          <div className="mb-1">
                            <label>{t('manpowerallocation_create_label_enddate')}</label>
                            <input type="date" name="EndDate" onChange={onUpdateField} value={user.EndDate ? formatDateTime(user.EndDate.toString()) : ""} className="form-control"></input>
                          </div>
                          <div className="col-md-12 mt-2">
                    ...
                    <div className=" mt-1">
                      <label className="form-label  mb-0">{t('create_user_label_engineercountryid')}</label>
                      <Select
                        options={Countries}
                    ...
                    <div className=" mt-2">
                      <label className="form-label  mb-0">{t('create_user_label_engineerstateid')}</label>
                      <Select
                        options={States}
                    ...
                    <div className=" mt-2">
                      <label className="form-label  mb-0">{t('create_user_label_engineercityid')}</label>
                      <Select
                        options={Cities}
                        name="EngineerCityId"
                    ...
                    <div className='mb-3 pt-2'>
                      <label>{t('create_user_label_engineerpincode')}</label>
                ...
                <div className='mt-2 mb-1'>
                  ...
                  />
                  <div className='small text-danger'> {errors['TenantOfficeId']}</div>
                </div>
