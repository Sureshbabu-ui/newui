...
import { ServiceRequestListState, changePage, loadServiceRequests, setFilter, updateField } from './AssetInterimRequestList.slice'
import Select from 'react-select';
import { Link } from "react-router-dom";
import FeatherIcon from 'feather-icons-react';
import { getPreAmcInterimServiceRequestList } from "../../../../../../services/serviceRequest";
...
export const AssetInterimRequestList = () => {
    const { t } = useTranslation();

    const onLoad = async () => {
        try {
            if (checkForPermission("SERVICE_REQUEST_ASSET_INTERIM_LIST")) {
    ...
    const { serviceRequests, totalRows, perPage, currentPage, filters, searchWith } = useStoreWithInitializer(({ assetinterimservicerequestlist }) => assetinterimservicerequestlist, onLoad);
    const [selectedFilter, setSelectedFilter] = useState<any>(null);

    const onPageChange = async (index: number) => {
        store.dispatch(changePage(index));
        if (checkForPermission("SERVICE_REQUEST_ASSET_INTERIM_LIST")) {
            const result = await getPreAmcInterimServiceRequestList(searchWith, index);
            store.dispatch(loadServiceRequests(result));
        }
    }
    ...
    const onUpdateField = async (ev: any) => {
        if (ev.target.value == "") {
            if (checkForPermission("SERVICE_REQUEST_ASSET_INTERIM_LIST")) {
                const result = await getPreAmcInterimServiceRequestList(searchWith, currentPage);
                store.dispatch(loadServiceRequests(result));
            }
        }
                            ...
                            {ServiceRequests.length > 0 ? (
                                <div className='ps-0 table-responsive overflow-auto pe-0'>
                                    <table className="table table-bordered text-nowrap">
                                        <thead>
                                            <tr>
                                                {checkForPermission("SERVICE_REQUEST_ASSET_INTERIM_LIST") &&
                                                    <th></th>
                                                }
                                        ...
                                        <tbody>
                                            {ServiceRequests.map(({ serviceRequest }, index) => (
                                                <tr className="mt-2">
                                                    {checkForPermission("SERVICE_REQUEST_ASSET_INTERIM_LIST") &&
                                                        <td>
                                                            <Link to={`/calls/asset/interimlist/${serviceRequest.Id}/review`} className="app-primary-color">
