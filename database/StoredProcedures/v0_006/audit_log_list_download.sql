...
BEGIN
	...
	SELECT @Columns = (SELECT STUFF((
		...
		CASE 
            ...
            THEN 'tb.' + QUOTENAME(COLUMN_NAME) + ' AT TIME ZONE ''India Standard Time'' AT TIME ZONE ''' + @TargetTimeZone + ''' AS ' + QUOTENAME(COLUMN_NAME)
            ELSE 'tb.' + QUOTENAME(COLUMN_NAME)
        END
	...
	-- Construct the dynamic query
		SET @Query = N'SELECT 
		CONVERT(VARCHAR(23), tm.tran_begin_time AT TIME ZONE ''India Standard Time'' AT TIME ZONE ''' + @TargetTimeZone + ''', 120) AS [Transaction Time], 
		tb.Id AS RowId,
